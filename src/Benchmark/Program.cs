...
namespace Benchmark {
    ...
    public class Benchmarks {
        ...
        public void TestRows() {
            using (var outer = new ConfigurationContainer(new BogusModule(), new LambdaParserModule()).CreateScope(@"files\bogus.xml?Size=5000")) {
        ...
        public void CSharpRows() {
            using (var outer = new ConfigurationContainer(new BogusModule(), new LambdaParserModule()).CreateScope(@"files\bogus-lambda-parser.xml?Size=5000")) {
                using (var inner = new TestContainer(new BogusModule(), new LambdaParserModule()).CreateScope(outer, new NullLogger())) {
                    var controller = inner.Resolve<IProcessController>();
                    controller.Execute();
                }
